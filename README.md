...
### お客さま側（LINE）
- **自動回答**：よくある質問（営業時間・定休日・支払い方法・アクセスなど）に AI が自動で答えます
- **文脈を理解**：直近の会話を踏まえて回答します（例：「それで、水曜は？」）
- **予約・修理受付**：「予約」「修理」と送ると、名前→希望日時→内容→電話番号を順に聞き取って受付します（途中で「キャンセル」と送れば中止できます）
- **画像受付**：故障箇所の写真を送るとスタッフへ引き継がれます

### スタッフ側（Django Admin）
- **会話履歴の確認**：ユーザーごとのやりとりを時系列で確認・検索できます
- **引き継ぎ対応**：AI が答えられなかった問い合わせは「要引き継ぎ」として一覧表示され、絞り込み・対応済み管理ができます
- **スタッフへの通知**：引き継ぎが発生すると担当スタッフの LINE に通知が届きます
- **FAQ / 店舗情報の管理**：FAQ の追加・編集・公開/非公開、店舗情報の編集ができます
- **予約の確認・確定**：受付された予約・修理依頼を確認し、最終確定できます

### 処理の流れ
```
LINE → Webhook → 署名検証 → 会話保存
    ├─ 質問 → FAQ/店舗情報 ＋ 会話履歴 → AI → 回答 or 引き継ぎ
    └─ 予約/修理/画像 → 受付フロー → 会話保存 → スタッフ確認
```
